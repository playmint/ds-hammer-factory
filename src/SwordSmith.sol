...
string constant ITEM_NAME = "Sword";
...
contract SwordSmith is BuildingKind {
    ...
    constructor(Game ds) {
        ...
        // Recipe
        inputItems[0] = Node.Resource(ResourceKind.IRON);
        inputQty[0] = 50;
        inputItems[1] = Node.Resource(ResourceKind.WOOD);
        inputQty[1] = 10;
    ...
    function use(Game ds, bytes24, /*buildingInstance*/ bytes24, /*seeker*/ bytes calldata payload) public {
        console.log("SwordSmith::use()");
        if (bytes4(payload) == ExtensionActions.CRAFT_ITEM.selector) {
            console.log("SwordSmith::use(): ExtensionActions.CRAFT_ITEM");
