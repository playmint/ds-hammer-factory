...
import {UnobtaniumFabricator} from "../src/UnobtaniumFabricator.sol";
...
contract Deployer is Script {
    ...
    function registerBuilding(Game ds, uint64 extensionID, bytes24 newItem) public returns (bytes24 buildingKind) {
        ...
        return BuildingUtils.register(ds, BuildingConfig({
            ...
            outputs: [
                Output({quantity: 2, item: newItem}) // this is the output that can be crafted given the inputs
            ],
            implementation: address(new UnobtaniumFabricator()),
            plugin: vm.readFile("src/UnobtaniumFabricator.js")
        }));
    }
}
