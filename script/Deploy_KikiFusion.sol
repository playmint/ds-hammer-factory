...
import {KikiFusion} from "../src/KikiFusion.sol";
...
contract Deployer is Script {
    ...
    function registerBuilding(Game ds, uint64 extensionID, bytes24 newItem) public returns (bytes24 buildingKind) {
        ...
        return BuildingUtils.register(ds, BuildingConfig({
            ...
            inputs: [
                Input({quantity: 100, item: superKiki}), // these are required inputs to get the outpu
                Input({quantity: 0, item: none}),
                Input({quantity: 0, item: none}),
                Input({quantity: 0, item: none})
            ],
            outputs: [
                Output({quantity: 5, item: newItem}) // this is the output that can be crafted given the inputs
            ],
            implementation: address(new KikiFusion()),
            plugin: vm.readFile("src/KikiFusion.js")
        }));
    }
}
